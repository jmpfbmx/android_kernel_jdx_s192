...
#define ARDBEG_TF_PWR	TEGRA_GPIO_PR0
...
static struct tegra_sdhci_platform_data tegra_sdhci_platform_data0 = {
	...
	.uhs_mask = MMC_UHS_MASK_DDR50 | MMC_UHS_MASK_SDR50 | MMC_UHS_MASK_SDR104 | MMC_UHS_MASK_SDR12 | MMC_UHS_MASK_SDR25,
//	.calib_3v3_offsets = 0x7676,
//	.calib_1v8_offsets = 0x7676,
	.max_clk_limit = 82000000,
	.disable_clock_gate = true,
	.thermal_support = false,//add by holman support to solve wifi crc error. 20151119
};
...
static int ardbeg_wifi_get_mac_addr(unsigned char *buf)
{
	...
	else
	{
		...
		if (rdlen > 0)
			fp->f_pos += rdlen;
		if (rdlen != 17)
			printk("%s: bad mac address file"" - len %d < 17",__func__, rdlen);
		else if (sscanf(str, "%x:%x:%x:%x:%x:%x", &mac[0], &mac[1], &mac[2], &mac[3], &mac[4], &mac[5]) != 6)
			printk("%s: bad mac address file - must contain xx:xx:xx:xx:xx:xx\n", __func__);
		else
		{
			printk("%s: using wifi mac %02x:%02x:%02x:%02x:%02x:%02x\n",__func__, 
		...
		(buf[3] == 0x00) &&  (buf[4] == 0x00) && (buf[5] == 0x00))
	{
		...
		printk("%s: using wifi random mac %02x:%02x:%02x:%02x:%02x:%02x\n",__func__, 
...
static int __init ardbeg_sdcard_init(void)
{
	int rc;

	rc = gpio_request(ARDBEG_TF_PWR, "sdcard_power");
	if (rc)
		pr_err("TF_PWR gpio request failed:%d\n", rc);

	rc = gpio_direction_output(ARDBEG_TF_PWR, 1);
	if (rc)
		pr_err("TF_PWR gpio direction configuration failed:%d\n", rc);
...
int __init ardbeg_sdhci_init(void)
{
	...
	else if (board_info.board_id == BOARD_E1784)
		tegra_sdhci_platform_data0.max_clk_limit = 204000000;
