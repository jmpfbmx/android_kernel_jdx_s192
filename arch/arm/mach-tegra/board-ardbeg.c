...
{
	ardbeg_bluedroid_pm_resources[0].start = ardbeg_bluedroid_pm_resources[0].end = gpio_to_irq(TEGRA_GPIO_PU6);
...
static struct rk30_adc_battery_platform_data rk30_adc_battery_platdata = {
		...
		.ctrl_charge_led_pin = TEGRA_GPIO_PK2, //red
		.ctrl_working_led_pin = TEGRA_GPIO_PK3,//green

		.reference_voltage = 1250, 
        .pull_up_res = 30,     //divider resistance ,  pull-up resistor
        .pull_down_res = 10, //divider resistance , pull-down resistor
...
static void ardbeg_usb_init(void)
{
	...
	if (board_info.sku == 1100 || board_info.board_id == BOARD_P1761 ||board_info.board_id == BOARD_E1780 \
...
static struct tegra_pingroup_config __initdata keys_pinmux[] = {
	DEFAULT_PINMUX(KB_COL6, RSVD1, PULL_UP, NORMAL, INPUT),
	DEFAULT_PINMUX(KB_COL7, RSVD1, PULL_UP, NORMAL, INPUT),
	DEFAULT_PINMUX(KB_COL0, RSVD1, PULL_UP, NORMAL, INPUT),
	DEFAULT_PINMUX(KB_ROW11, RSVD1, PULL_UP, NORMAL, INPUT),
	DEFAULT_PINMUX(KB_ROW12, RSVD1, PULL_UP, NORMAL, INPUT),
	DEFAULT_PINMUX(KB_ROW13, RSVD1, PULL_UP, NORMAL, INPUT),
	DEFAULT_PINMUX(KB_ROW14, RSVD1, PULL_UP, NORMAL, INPUT),
	DEFAULT_PINMUX(KB_ROW15, RSVD2, PULL_UP, NORMAL, INPUT),
	DEFAULT_PINMUX(GPIO_PI2, RSVD3, PULL_UP, NORMAL, INPUT),
	DEFAULT_PINMUX(KB_COL2, RSVD1, PULL_UP, NORMAL, INPUT),
	DEFAULT_PINMUX(KB_COL3, KBC, PULL_UP, NORMAL, INPUT),
	DEFAULT_PINMUX(KB_COL1, RSVD1, PULL_UP, NORMAL, INPUT),
	DEFAULT_PINMUX(KB_COL4, KBC, PULL_UP, NORMAL, INPUT),
	DEFAULT_PINMUX(KB_ROW9, RSVD1, PULL_UP, NORMAL, INPUT),
	DEFAULT_PINMUX(KB_ROW10, RSVD1, PULL_UP, NORMAL, INPUT),
	DEFAULT_PINMUX(KB_ROW6, KBC, PULL_UP, NORMAL, INPUT),
	//DEFAULT_PINMUX(KB_ROW7, RSVD1, PULL_UP, NORMAL, INPUT),
	DEFAULT_PINMUX(KB_ROW3, KBC, PULL_UP, NORMAL, INPUT),
};

int __init keys_pinmux_init(void)
{
	tegra_pinmux_config_table(keys_pinmux, ARRAY_SIZE(keys_pinmux));
	return 0;
}

#ifdef CONFIG_TOUCHSCREEN_FT5X06
static struct tegra_pingroup_config __initdata ft5x06_pinmux[] = {
	DEFAULT_PINMUX(SPDIF_IN,  RSVD1, NORMAL, NORMAL, OUTPUT),
	DEFAULT_PINMUX(SPDIF_OUT, RSVD1, NORMAL, NORMAL, INPUT),
};

#define TOUCH_FT5X06_INT		TEGRA_GPIO_PK5
#define TOUCH_FT5X06_RST		TEGRA_GPIO_PK6

struct edt_ft5x06_platform_data __initdata ft5x06_platform_data = {
	.irq_pin = TOUCH_FT5X06_INT,
	.reset_pin = TOUCH_FT5X06_RST,
};

static struct i2c_board_info __initdata ft5x06_board_info = {
	I2C_BOARD_INFO("ft5x06-touch", 0x38),
	.irq = -1,
	.platform_data = &ft5x06_platform_data,
};

int __init touch_init_ft5x06(void)
{
	int err = 0;
	tegra_pinmux_config_table(ft5x06_pinmux, ARRAY_SIZE(ft5x06_pinmux));

	ft5x06_board_info.irq = gpio_to_irq(TOUCH_FT5X06_INT);
	i2c_register_board_info(1, &ft5x06_board_info, 1);
...
#if 1
static struct tegra_pingroup_config __initdata bma250_pinmux[] = {
	DEFAULT_PINMUX(KB_ROW8, RSVD1, NORMAL, NORMAL, INPUT),
};

#define BMA250_INT		TEGRA_GPIO_PS0


struct edt_ft5x06_platform_data __initdata bma250_platform_data = {
	.irq_pin = BMA250_INT,
};


static struct i2c_board_info __initdata bma250_board_info = {
	I2C_BOARD_INFO("bma250", 0x18),
	.irq = -1,
	.platform_data = &bma250_platform_data,
};

int __init gsensor_init_bma250(void)
{
	int err = 0;
	tegra_pinmux_config_table(bma250_pinmux, ARRAY_SIZE(bma250_pinmux));

	bma250_board_info.irq = gpio_to_irq(BMA250_INT);
	printk("bma250,register i2c devices\n");
	i2c_register_board_info(0, &bma250_board_info, 1);
...
static int ardbeg_gsensors_init(void)
{
//#ifdef CONFIG_INPUT_BMA250
#if 1
	gsensor_init_bma250();
...
static struct tegra_pingroup_config k1_config[]={
...
void led_red_ops(int value)
{
	struct regulator *regulator = regulator_get(NULL,"vpp_red_led");
	if (!IS_ERR(regulator)){
		...
		printk("led_ops: error to get red regulator \n");
	
}

void led_green_ops(int value)
{
	struct regulator *regulator = regulator_get(NULL,"vpp_green_led");
	if (!IS_ERR(regulator)){
		...
		printk("led_ops: error to get green regulator \n");
	
}

void led_ops_func(enum led_type type,int value)
{
	if (type == RED){
		led_red_ops(value);
...
static void __init tegra_ardbeg_late_init(void)
{
	...
	tegra_pinmux_config_table(k1_config,ARRAY_SIZE(k1_config));
	//gpio_direction_output(TEGRA_GPIO_PR4,0);   kb_row4 , 40pin lcd backlight vs , vgh en
	//gpio_direction_output(TEGRA_GPIO_PH3,0);   40pin lcd reset pin

	ret = gpio_request(TEGRA_GPIO_PR4, "vgh_en");
    if (ret < 0)
    {
        printk("gpio vgh_en can't request\n");
    }
    ret = gpio_direction_output(TEGRA_GPIO_PR4, 0);
    if (ret < 0) {
        gpio_free(TEGRA_GPIO_PR4);
		printk("free gpio vgh_en\n");
    }

	ret = gpio_request(TEGRA_GPIO_PH3, "lcd_rst");
    if (ret < 0)
    {
        printk("gpio lcd_rst can't request\n");
    }
    ret = gpio_direction_output(TEGRA_GPIO_PH3, 0);
    if (ret < 0) {
        gpio_free(TEGRA_GPIO_PH3);
		printk("free gpio lcd_rst\n");
    }
	...
	ardbeg_sensors_init();//yunboa
	ardbeg_gsensors_init();
