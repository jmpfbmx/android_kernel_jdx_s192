...
static int ardbeg_dw9718_power_on(struct dw9718_power_rail *pw)
{
	...
	if (unlikely(!pw || !pw->vdd1 || !pw->vdd_i2c))
		return -EFAULT;
	gpio_set_value(CAM1_PWDN, 0);

	err = regulator_enable(pw->vdd1);
	if (unlikely(err))
		goto dw9718_vdd_fail;
	err = regulator_enable(pw->vdd2);
	if (unlikely(err))
		goto dw9718_vdd_fail;
	err = regulator_enable(pw->vdd3);
	if (unlikely(err))
		goto dw9718_vdd_fail;
...
static int ardbeg_dw9718_power_off(struct dw9718_power_rail *pw)
{
	...
	gpio_set_value(CAM1_PWDN, 0);
	gpio_set_value(CAM_AF_PWDN, 0);
	regulator_disable(pw->vdd3);
	regulator_disable(pw->vdd2);
	regulator_disable(pw->vdd1);
...
static int ardbeg_ov5693_power_on(struct ov5693_power_rail *pw)
{
	...
	gpio_set_value(CAM2_PWDN, 0);
	usleep_range(10, 20);
	err = regulator_enable(pw->extvdd);
	if (err)
...
static struct nvc_imager_cap ov5693_front_cap = {
	.identifier				= "OV5693.1",
	.sensor_nvc_interface	= 5,                 // a , b, c, : 3, 4, 5
	.pixel_types[0]			= 0x101,
	.orientation			= 0,
	.direction				= 0,
	.initial_clock_rate_khz	= 6000,
...
#define CAM_RSTN_2529 143
static int ardbeg_sp2529_power_on(struct sp2529_power_rail *pw)
{
	pr_info("%s\n", __func__);
	...
	tegra_io_dpd_disable(&csid_io);
	mdelay(10);
	if (regulator_enable(pw->avdd))
		goto sp2529_avdd_fail;
	mdelay(10);
	if (regulator_enable(pw->dovdd))
		goto sp2529_aovdd_fail;
	mdelay(10);
	if (regulator_enable(pw->dvdd))
		goto sp2529_aovdd_fail;
	mdelay(200);
	gpio_set_value(CAM2_PWDN, 0);
	mdelay(50);
	...
	gpio_set_value(CAM2_PWDN, 0);
	mdelay(200);
	gpio_set_value(CAM_RSTN_2529, 1);
	mdelay(50);
	gpio_set_value(CAM_RSTN_2529, 0);
	mdelay(50);
	gpio_set_value(CAM_RSTN_2529, 1);
	mdelay(50);
	pr_info("[pengge]::::::::::%s\n", __func__);
...
static int ardbeg_sp2529_power_off(struct sp2529_power_rail *pw)
{
	pr_info("%s\n", __func__);
...
static struct sp2529_platform_data ardbeg_sp2529_pdata = {
	.mclk_name	= "mclk2",
	.power_on	= ardbeg_sp2529_power_on,
	.power_off	= ardbeg_sp2529_power_off,
};
...
static struct i2c_board_info	ardbeg_i2c_board_info_ad5816 = {
	I2C_BOARD_INFO("dw9716", 0x0e),
	.platform_data = &ardbeg_dw9718_data,
};
...
static struct i2c_board_info	ardbeg_i2c_board_info_sp2529 = {
	I2C_BOARD_INFO("sp2529", 0x30),
	.platform_data = &ardbeg_sp2529_pdata,
};
...
static struct camera_module ardbeg_camera_module_info[] = {
	...
	/* E1793 camera board */
	{
		...
		.focuser = &ardbeg_i2c_board_info_ad5816,
		//.flash = &ardbeg_i2c_board_info_as3648,
	},
	{
		/* front camera */
	...
	{
		/* front camera */
		.sensor = &ardbeg_i2c_board_info_sp2529,
	},
	{}
};
...
static struct gadc_thermal_platform_data gadc_thermal_vbat_pdata = {
	.iio_channel_name = "vbat_channel",
	.tz_name = "battery-voltage",
	.temp_offset = 303,  //190mv ==>adc value,
	.adc_to_temp = NULL,
};
...
static struct platform_device gadc_thermal_vbat = {
	.name   = "generic-adc-thermal",
	.id     = 3,
	.dev	= {
		.platform_data = &gadc_thermal_vbat_pdata,
	},
};
