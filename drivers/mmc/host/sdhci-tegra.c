...
/*Reduces freq to High(from max) if Temp exceeds defined trip point*/
#define NVQUIRK_ENABLE_RELIABILITY_MODE		BIT(25)
...
#define RELIABILITY_FREQ	136000000
...
static void sdhci_tegra_handle_reliability_mode(struct sdhci_host *sdhci,
				int thermal_index);

#ifdef CONFIG_THERMAL
static int sdhci_mmc_temperature[] = {62};
static int sdhci_sd_temperature[] = {62};
static int sdhci_sdio_temperature[] = {62};
...
struct sdhci_thermal {
	struct tegra_cooling_device *sdhci_cd;
	int thermal_index;
	int instance;
	struct sdhci_host *thermal_sdhci;
};

struct sdhci_thermal sdhci_td[] = {
	{
		.sdhci_cd = &sdhci_cdev[0],
		.thermal_index = 0,
		.instance = 0,
	},
	{
		.sdhci_cd = &sdhci_cdev[1],
		.thermal_index = 0,
		.instance = 1,
	},
	{
		.sdhci_cd = &sdhci_cdev[2],
		.thermal_index = 0,
		.instance = 2,
	},
};
...
#define TUNING_STATUS_RETUNE	2
	...
	struct tegra_tuning_data rel_mode_tuning_data;
	u32 speedo;
	bool rel_tuning_done;
	bool thermal_support;
	bool rel_freq_change;
	int prev_thermal_idx;
};

#ifdef CONFIG_THERMAL
struct tegra_cooling_device *tegra_sdhci_get_cdev(int instance)
{
			...
			unsigned long cur_state)
{
	...
	if (sdhci_data->thermal_sdhci)
		sdhci_tegra_handle_reliability_mode(sdhci_data->thermal_sdhci,
			sdhci_data->thermal_index);
	...
	struct tegra_freq_gov_data *gov_data)
{
	...
	if (!cur_state && gov_data->curr_active_load >=
		...
		if ((cur_state && desired_freq ==
			gov_data->freqs[TUNING_HIGH_FREQ])
	...
	unsigned int clock)
{
	...
	if (((sdhci->mmc->df) || (tegra_host->rel_freq_change)) &&
		(tegra_host->tuning_status == TUNING_STATUS_DONE)) {
		...
		tegra_host->rel_freq_change = false;
	}
}
	...
	struct sdhci_host *sdhci, unsigned int clock)
{
	...
	if (tegra_host->max_clk_limit &&
		(clock > tegra_host->max_clk_limit))
		clock = tegra_host->max_clk_limit;

	if ((soc_data->nvquirks & NVQUIRK_ENABLE_RELIABILITY_MODE)
		&& (!(sdhci->mmc->caps2 & MMC_CAP2_FREQ_SCALING))
		&& (tegra_host->rel_tuning_done)
		&& (tegra_host->thermal_support)) {
		tuning_data = &tegra_host->rel_mode_tuning_data;
		if (clock > tuning_data->freq_hz)
		...
		bool reliability_mode)
{
	...
	for (i = 0; i < tegra_host->tuning_freq_count; ++i) {
		if (reliability_mode) {
			tuning_data = &tegra_host->rel_mode_tuning_data;
			i = tegra_host->tuning_freq_count;
		} else {
...
static int find_tuning_coeffs_data(struct sdhci_host *sdhci,
		bool force_retuning, bool reliability_mode)
{
	struct sdhci_pltfm_host *pltfm_host = sdhci_priv(sdhci);
	struct sdhci_tegra *tegra_host = pltfm_host->priv;
	const struct sdhci_tegra_soc_data *soc_data = tegra_host->soc_data;
							...
							bool force_retuning)
{
	...
	err = find_tuning_coeffs_data(sdhci, force_retuning, false);
	if (err)
		return err;

	sdhci_tegra_dump_tuning_constraints(sdhci, false);
		...
		struct tegra_tuning_data *tuning_data)
{
	...
	if (err)
		dev_err(mmc_dev(sdhci->mmc),
			"%dMHz best tap value = %d verification failed %d\n",
			tuning_data->freq_hz, tuning_data->best_tap_value, err);
	return err;
}

static int sdhci_tegra_start_tuning(struct sdhci_host *sdhci,
			struct tegra_tuning_data *tuning_data)
{
	struct sdhci_pltfm_host *pltfm_host = sdhci_priv(sdhci);
	struct sdhci_tegra *tegra_host = pltfm_host->priv;
	...
	err = sdhci_tegra_verify_best_tap(sdhci, tuning_data);

tuning_out:
	return err;
}

static int calculate_reliablity_mode_tap(struct sdhci_host *sdhci)
{
	int err = 0, i, freq;
	struct sdhci_pltfm_host *pltfm_host = sdhci_priv(sdhci);
	struct sdhci_tegra *tegra_host = pltfm_host->priv;
	struct tegra_tuning_data *rel_tuning_data;

	rel_tuning_data = &tegra_host->rel_mode_tuning_data;

	rel_tuning_data->freq_hz = RELIABILITY_FREQ;
	rel_tuning_data->freq_band = 1;
	rel_tuning_data->constraints.vcore_mask =
		tuning_vcore_constraints[1].vcore_mask;
	rel_tuning_data->nr_voltages =
		hweight32(rel_tuning_data->constraints.vcore_mask);

	for (i = 0; i < tegra_host->tuning_freq_count; i++) {
		freq = tegra_host->tuning_data[i].freq_hz;
		if (freq == rel_tuning_data->freq_hz) {
			rel_tuning_data->best_tap_value =
				tegra_host->tuning_data[i].best_tap_value;
			dev_info(mmc_dev(sdhci->mmc),
			"Reliability mode: %dMHz Tuning is already done, best tap %d\n",
			rel_tuning_data->freq_hz,
			rel_tuning_data->best_tap_value);
			goto reliablity_mode_tap_out;
		}
	}

	err = find_tuning_coeffs_data(sdhci, true, true);
	if (err) {
		dev_err(mmc_dev(sdhci->mmc),
	...
	sdhci_tegra_dump_tuning_constraints(sdhci, true);
	err = sdhci_tegra_start_tuning(sdhci, rel_tuning_data);

reliablity_mode_tap_out:
	return err;
}
...
static int sdhci_tegra_execute_tuning(struct sdhci_host *sdhci, u32 opcode)
{
	...
	for (i = 0; i < tegra_host->tuning_freq_count; i++) {
		...
		err = sdhci_tegra_start_tuning(sdhci, tuning_data);
		if (!err) {
			tuning_data->tuning_done = true;
			if (i == (tegra_host->tuning_freq_count - 1))
				tegra_host->tuning_status |= TUNING_STATUS_DONE;
		} else {
			tegra_host->tuning_status |= TUNING_STATUS_RETUNE;
			goto out;
	...
	if ((soc_data->nvquirks & NVQUIRK_ENABLE_RELIABILITY_MODE) &&
				(tegra_host->thermal_support)) {
		err = calculate_reliablity_mode_tap(sdhci);
		if (err) {
			goto out;
		} else if (tegra_host->tuning_status == TUNING_STATUS_DONE) {
			i = (tegra_host->tuning_freq_count - 1);
			tuning_data = &tegra_host->tuning_data[i];
			...
			sdhci_tegra_set_tap_delay(sdhci,
				tuning_data->best_tap_value);
			spin_unlock(&sdhci->lock);
			tegra_host->rel_tuning_done = true;
		}
	}
out:
				...
				int thermal_index)
{
	...
	struct sdhci_tegra *tegra_host = pltfm_host->priv;
	const struct sdhci_tegra_soc_data *soc_data = tegra_host->soc_data;
	struct tegra_tuning_data *tuning_data = NULL;

	/* No change in thermal state ...just exit */
	if (tegra_host->prev_thermal_idx == thermal_index)
		return;
	...
	if (sdhci->max_clk >= RELIABILITY_FREQ) {
		if (thermal_index == 1)
			tuning_data = &tegra_host->rel_mode_tuning_data;
		else if (thermal_index == 0)
			tuning_data = &tegra_host->tuning_data[0];
		tegra_host->rel_freq_change = true;
	}
	if (tegra_host->rel_freq_change) {
		tegra_host->prev_thermal_idx = thermal_index;
		tegra_host->max_clk_limit = tuning_data->freq_hz;
		sdhci->mmc->ios.clock = tuning_data->freq_hz;
		dev_info(mmc_dev(sdhci->mmc),
			"Thermal state: %d, Changing freq to %uhz in next transaction\n",
				thermal_index, tuning_data->freq_hz);
	}
}
...
static struct sdhci_tegra_soc_data soc_data_tegra12 = {
	...
	.nvquirks = TEGRA_SDHCI_NVQUIRKS |
		    ...
		    NVQUIRK_ENABLE_RELIABILITY_MODE |
		    NVQUIRK_DISABLE_EXTERNAL_LOOPBACK,
...
static int sdhci_tegra_probe(struct platform_device *pdev)
{
	...
	unsigned int thermal_cdev_arr_index = 0;
	int rc;
	u8 i;
	...
	tegra_host->thermal_support = plat->thermal_support;
...
#ifdef CONFIG_THERMAL
	if (tegra_host->instance >= 2)
		thermal_cdev_arr_index = tegra_host->instance - 1;
	if (tegra_host->thermal_support) {
		tegra_host->cdev = thermal_cooling_device_register(
		sdhci_cdev[thermal_cdev_arr_index].cdev_type,
		&sdhci_td[thermal_cdev_arr_index], &sdhci_tegra_cooling_ops);
		if (IS_ERR_OR_NULL(tegra_host->cdev)) {
			dev_err(mmc_dev(host->mmc),
			":Thermal cooling device registration has failed\n");
			if (plat->power_off_rail)
				unregister_reboot_notifier(
				&tegra_host->reboot_notify);
			goto err_cd_irq_req;
		}
		sdhci_td[thermal_cdev_arr_index].thermal_sdhci = host;
	}
#endif
