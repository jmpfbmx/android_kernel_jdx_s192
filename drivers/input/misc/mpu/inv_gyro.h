...
struct inv_gyro_state_s {
	...
	atomic_t delay;//add by yunboa
	s16 bma_data[3];
	struct work_struct work_struct;
	struct delayed_work work; //add by yunboa
};
...
struct inv_mpu_slave {
	...
	int (*read_bytes)(struct inv_gyro_state_s *,
...
struct bma250acc {
	char first_time;
	char num;
	s16 data[3][SAMPLE_MAX];
	s16 x, y, z;
	s16 pre_x, pre_y, pre_z;
	s16 pre_val[3];
	s16 pre_average_val[3];
};
...
int bma250_read_accel_xyz(struct inv_gyro_state_s *st, struct bma250acc *acc);
