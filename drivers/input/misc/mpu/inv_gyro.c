...
static u16 nvi_report_accl(struct inv_gyro_state_s *inf, u8 *data, s64 ts)
{
	...
	}else if(inf->chip_type == INV_MPU6050){
		if (inf->mpu_slave != NULL) {
			inf->mpu_slave->combine_data(data, raw);
			for (i = 0; i < 3; i++)
				val[i] = raw[i];
...
static void nvi_sync(struct inv_gyro_state_s *inf, s64 ts)
{
	...
	input_report_abs(inf->idev, ABS_MISC, (unsigned int)(ts >> 32));
	input_report_abs(inf->idev, ABS_WHEEL,
			 (unsigned int)(ts & 0xffffffff));
	input_sync(inf->idev);
}
			   ...
			   struct i2c_client *client, unsigned char *name)
{
	...
	input_set_capability(idev, EV_ABS, ABS_Z);
	
	input_set_abs_params(idev, ABS_X, ABSMIN_2G, ABSMAX_2G, 0, 0);
	input_set_abs_params(idev, ABS_Y, ABSMIN_2G, ABSMAX_2G, 0, 0);
	input_set_abs_params(idev, ABS_Z, ABSMIN_2G, ABSMAX_2G, 0, 0);
			...
			const struct i2c_device_id *id)
{
	...
	case INV_MPU6050:
		...
		inv_register_bma250_slave(inf);
		if (inf->mpu_slave != NULL) {
			err = inf->mpu_slave->setup(inf);
			if (err)
				return err;
		}
		break;
...
static int nvi_suspend(struct device *dev)
{
	...
	//cancel_delayed_work_sync(&inf->work);//add by yunboa
	mutex_unlock(&inf->mutex);
	if (err)
		dev_err(dev, "%s ERR\n", __func__);
	if (inf->dbg & NVI_DBG_SPEW_MSG)
...
static int nvi_resume(struct device *dev)
{
	...
	BUG_ON(!inf->suspend);
	inf->suspend = false;
	/*schedule_delayed_work(&inf->work,
				      msecs_to_jiffies(atomic_read
						       (&inf->delay)));//add by yunboa*/
	mutex_unlock(&inf->mutex);
	if (inf->dbg & NVI_DBG_SPEW_MSG)
		dev_info(dev, "%s done\n", __func__);
	return 0;
				...
				struct mpu_platform_data *pdata)
{
	...
	if (!of_property_read_u32_array(np, "invensense,secondary_i2c_addr", &tmp, 1))
		pdata->secondary_i2c_addr = (u16)tmp;
	else
	{	
		dev_err(&client->dev, "Can't read secondary_i2c_addr property\n");
	}
...
{
	struct inv_gyro_state_s *st = container_of((struct delayed_work *)work,
						  struct inv_gyro_state_s, work);
	static struct bma250acc acc;
	s64 ts;
	
	unsigned long delay = msecs_to_jiffies(atomic_read(&st->delay));
	ts = nvi_ts_ns();
	bma250_read_accel_xyz(st, &acc);
	if((abs(st->bma_data[0] - acc.x) > GSENSOR_MIN)||(abs(st->bma_data[1] - acc.y) > GSENSOR_MIN)||(abs(st->bma_data[2] - acc.z) > GSENSOR_MIN))
	{
		input_report_abs(st->idev, ABS_X, -acc.x * 4 * 17);
		input_report_abs(st->idev, ABS_Y, -acc.y * 4 * 17);
		input_report_abs(st->idev, ABS_Z, acc.z * 4 * 17);
		st->bma_data[0] = acc.x;
		st->bma_data[1] = acc.y;
		st->bma_data[2] = acc.z;
		nvi_sync(st, ts);
		     ...
		     const struct i2c_device_id *id)
{
	...
	atomic_set(&st->delay, 10);
	INIT_DELAYED_WORK(&st->work, bma250_work_func);
	...
	//schedule_delayed_work(&st->work,msecs_to_jiffies(atomic_read(&st->delay)));
