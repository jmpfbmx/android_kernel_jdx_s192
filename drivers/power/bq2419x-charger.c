	...
	int in_current_limit)
{
	...
	val = current_to_reg(iinlim, ARRAY_SIZE(iinlim), 3000);
	...
	bq2419x->in_current_limit = 3000;
	return ret;
}


#define MAX_CHARING_CURRENT   3000000
			...
			int min_uA, int max_uA)
{
	...
	if ((val & BQ2419x_VBUS_STAT) == BQ2419x_VBUS_UNKNOWN) {
		dev_info(bq2419x->dev, "getting unknow tyep \n");
		battery_charging_restart_cancel(bq2419x->bc_dev);
		in_current_limit = MAX_CHARING_CURRENT;
		bq2419x->cable_connected = 0;
		bq2419x->chg_status = BATTERY_DISCHARGING;
		battery_charger_thermal_stop_monitoring(
				bq2419x->bc_dev);
	} else {
	dev_info(bq2419x->dev, "getting usbtype tyep \n");
...
static int bq2419x_suspend(struct device *dev)
{
	...
	if (bq2419x->cable_connected &&
		...
		ret = bq2419x_set_charging_current_suspend(bq2419x, MAX_CHARING_CURRENT);
		if (ret < 0)
			dev_err(bq2419x->dev,
