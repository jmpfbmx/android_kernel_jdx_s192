...
static char * mode_name[] = {
	"ov5693_2592x1944_i2c",// ==> OV5693_MODE_2592x1944
	"ov5693_1920x1080_i2c",
	"ov5693_1296x972_i2c",
	"ov5693_1280x720_120fps_i2c",
	"ov5693_2592x1944_HDR_24fps_i2c",
	"ov5693_1920x1080_HDR_30fps_i2c",
	"ov5693_1296x972_HDR_30fps_i2c",
	"ov5693_1280x720_HDR_60fps_i2c"
};
static int ov5693_i2c_rd_table(struct ov5693_info *info)
{
	...
	u16 i2c_reg = 0;
	u8 i2c_buf[OV5693_SIZEOF_I2C_BUF];
	u16 reg_addr_buf[OV5693_SIZEOF_I2C_BUF];

	if(info->mode_index < sizeof(mode_table)/sizeof(mode_table[0]))		
		//printk("nathanz: %s - %s - mode:%s - %d \n",__FILE__,__func__,mode_name[info->mode_index],__LINE__);

	for (next = mode_table[info->mode_index]; next->addr != OV5693_TABLE_END; next++) {
		if (next->addr == OV5693_TABLE_WAIT_MS) {
			msleep(next->val);
			continue;
		} else if (next->addr == OV5693_TABLE_RESET) {
		...
		if (buf_count == 0) {
			memset(i2c_buf,0,OV5693_SIZEOF_I2C_BUF);
			memset(reg_addr_buf,0,OV5693_SIZEOF_I2C_BUF);
			i2c_reg = next->addr;
		}

		reg_addr_buf[buf_count] = next->addr;
		buf_count++;
		n_next = next + 1;
		if (n_next->addr == next->addr + 1 &&
			n_next->addr != OV5693_TABLE_WAIT_MS &&
		...
		err = regmap_bulk_read(info->regmap, i2c_reg, i2c_buf, buf_count);
		if (err)
			return err;
		for(i=0 ; i<buf_count ; i++)
			printk("reg = 0x%x , val = 0x%x\n",reg_addr_buf[i],i2c_buf[i]);
				...
				struct ov5693_ae *ae)
{
	...
	reg_list[offset].addr = OV5693_TABLE_END;
	err |= ov5693_i2c_wr_table(info, reg_list);
	printk("%s   %s : gain=%d,  frame_lengt=%d ,  coarse_time=%d ,- %d\n",__FILE__,__func__,ae->gain_enable,ae->frame_length_enable,ae->coarse_time_enable,__LINE__);
	return err;
}
...
static int ov5693_power_off(struct ov5693_info *info)
{
	...
	int err;
	//printk("nathanz: %s -enter %s - %d\n",__FILE__,__func__,__LINE__);
	//ov5693_i2c_rd_table(info);
...
static void ov5693_pm_init(struct ov5693_info *info)
{
	...
	ov5693_regulator_get(info, &pw->extvdd, info->pdata->regulators.extvdd);
...
static int ov5693_mode_wr_full(struct ov5693_info *info, u32 mode_index)
{
	...
	//printk("nathanz: %s - %s - %d ,before write register ======\n",__FILE__,__func__,__LINE__);
	//ov5693_i2c_rd_table(info);
	...
	//printk("nathanz: %s - %s - %d ,after write register ======\n",__FILE__,__func__,__LINE__);
...
static long ov5693_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	int err;
	//printk("nathanz: %s - %s - %d\n",__FILE__,__func__,__LINE__);
	dev_dbg(&info->i2c_client->dev, "%s ioctl: %d , arg=%d\n",
	...
	const struct i2c_device_id *id)
{
	...
	//memcpy(clk_name,mclk_name,strlen(mclk_name));
