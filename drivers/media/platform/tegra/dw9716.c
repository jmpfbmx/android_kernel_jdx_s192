/*
 * dw9716.c - dw9716 focuser driver
 *
 ...
 * .info = Pointer to the dw9716_pdata_info structure.  This structure does
...
#define dw9716_ID			0x04
#define dw9716_FOCAL_LENGTH_FLOAT	(4.570f)
#define dw9716_FNUMBER_FLOAT		(2.8f)
#define dw9716_FOCAL_LENGTH		(0x40923D71) /* 4.570f */
#define dw9716_FNUMBER			(0x40333333) /* 2.8f */
#define dw9716_SLEW_RATE		0x080002
#define dw9716_ACTUATOR_RANGE		1023
#define dw9716_SETTLETIME		10
#define dw9716_FOCUS_MACRO		620
#define dw9716_FOCUS_INFINITY		70
#define dw9716_POS_LOW_DEFAULT		0
#define dw9716_POS_HIGH_DEFAULT		1023
#define dw9716_POS_CLAMP		0x03ff
/* Need to decide exact value of VCM_THRESHOLD and its use */
/* define dw9716_VCM_THRESHOLD	20 */

struct dw9716_info {
	struct i2c_client *i2c_client;
	struct regmap *regmap;
	struct camera_sync_dev *csync_dev;
	struct dw9716_platform_data *pdata;
	struct miscdevice miscdev;
	struct list_head list;
	struct dw9716_power_rail power;
	struct dw9716_info *s_info;
 ...
 */
static struct nvc_focus_cap dw9716_default_cap = {
	.version = NVC_FOCUS_CAP_VER2,
	.slew_rate = dw9716_SLEW_RATE,
	.actuator_range = dw9716_ACTUATOR_RANGE,
	.settle_time = dw9716_SETTLETIME,
	.focus_macro = dw9716_FOCUS_MACRO,
	.focus_infinity = dw9716_FOCUS_INFINITY,
	.focus_hyper = dw9716_FOCUS_INFINITY,
};

static struct nvc_focus_nvc dw9716_default_nvc = {
	.focal_length = dw9716_FOCAL_LENGTH,
	.fnumber = dw9716_FNUMBER,
	.max_aperature = dw9716_FNUMBER,
};

static struct dw9716_platform_data dw9716_default_pdata = {
	.cfg = 0,
	.num = 0,
	.sync = 0,
	.dev_name = "focuser",
};
static LIST_HEAD(dw9716_info_list);
static DEFINE_SPINLOCK(dw9716_spinlock);

static int dw9716_i2c_wr8(struct dw9716_info *info, u8 reg, u8 val)
{
	struct i2c_msg msg;
	u8 buf[2];
	buf[0] = reg;
...
static int dw9716_i2c_wr16(struct dw9716_info *info, u8 reg, u16 val)
{
	struct i2c_msg msg;
	u8 buf[3];
	buf[0] = reg;
...
static int dw9716_i2c_rd8(struct dw9716_info *info, u8 reg, u8 *val)
{
 ...
 */
static int dw9716_position_wr(struct dw9716_info *info, s32 position)
{
	int err;

	dev_dbg(&info->i2c_client->dev, "%s %d\n", __func__, position);
	position &= dw9716_POS_CLAMP;
#if 0
...
#else
	err = dw9716_i2c_wr8(info, 0x3, ((position>>8)&0x3));
	if (!err)
		info->cur_pos = position;
	else
		dev_err(&info->i2c_client->dev, "%s: ERROR set position %d",
			__func__, position);
	err = dw9716_i2c_wr8(info, 0x4, (position &0xff));
	if (!err)
		info->cur_pos = position;
	else
		dev_err(&info->i2c_client->dev, "%s: ERROR set position %d",
...
int dw9716_set_arc_mode(struct dw9716_info *info)
{
	int err;
	...
	/* set ARC enable */
	err = dw9716_i2c_wr8(info, DW9716_CONTROL, (sr >> 16) & 0xFF);
	if (err) {
	...
	/* set the ARC RES2 */
	err = dw9716_i2c_wr8(info, DW9716_SWITCH_MODE, (sr >> 8) & 0xFF);
	if (err) {
		dev_err(&info->i2c_client->dev,
		"%s: MODE write failed\n", __func__);
		goto set_arc_mode_done;
	...
	err = dw9716_i2c_wr8(info, DW9716_SACT, sr & 0XFF);
	if (err) {
		dev_err(&info->i2c_client->dev,
		"%s: RES write failed\n", __func__);
		goto set_arc_mode_done;
...
static int dw9716_pm_wr(struct dw9716_info *info, int pwr)
{
	int err = 0;
...
static int dw9716_power_put(struct dw9716_power_rail *pw)
{
	if (unlikely(!pw))
		return -EFAULT;

	if (likely(pw->vdd1))
		regulator_put(pw->vdd1);
	if (likely(pw->vdd2))
		regulator_put(pw->vdd2);
	if (likely(pw->vdd1))
		regulator_put(pw->vdd3);
...
static int dw9716_regulator_get(struct dw9716_info *info,
...
static int dw9716_power_get(struct dw9716_info *info)
{
	struct dw9716_power_rail *pw = &info->power;

	dw9716_regulator_get(info, &pw->vdd1, "vdd_cam_1v1_cam");
	dw9716_regulator_get(info, &pw->vdd2, "vdd_lcd_1v2_s");
	dw9716_regulator_get(info, &pw->vdd3, "imx135_reg2");
	dw9716_regulator_get(info, &pw->vdd_i2c, "vdd_2v7_hv");

	return 0;
}

static int dw9716_pm_dev_wr(struct dw9716_info *info, int pwr)
{
	return dw9716_pm_wr(info, pwr);
}

static void dw9716_pm_exit(struct dw9716_info *info)
{
	dw9716_pm_wr(info, NVC_PWR_OFF_FORCE);
	dw9716_power_put(&info->power);
}

static int dw9716_reset(struct dw9716_info *info, u32 level)
{
	int err = 0;

	if (level == NVC_RESET_SOFT) {
		err = dw9716_i2c_wr8(info, 0x03, 200);
		usleep_range(200, 220);
		err |= dw9716_i2c_wr8(info, 0x03, 100);
		usleep_range(100, 120);
	} else
		err = dw9716_pm_wr(info, NVC_PWR_OFF_FORCE);

	return err;
}

static int dw9716_detect(struct dw9716_info *info)
{
	u8 val = 0;
	int err;

	dw9716_pm_dev_wr(info, NVC_PWR_COMM);
	err = dw9716_i2c_rd8(info, 0, &val);
	dw9716_pm_dev_wr(info, NVC_PWR_OFF);
	return err;
}

static void dw9716_get_focuser_capabilities(struct dw9716_info *info)
{
	...
	info->nv_config.pos_actual_low = dw9716_POS_LOW_DEFAULT;
	info->nv_config.pos_actual_high = dw9716_POS_HIGH_DEFAULT;
...
static int dw9716_set_focuser_capabilities(struct dw9716_info *info,
					struct nvc_param *params)
{
	...
	if (info->pdata->cap->settle_time)
		...
		info->cap.settle_time = dw9716_SETTLETIME;
...
static int dw9716_param_rd(struct dw9716_info *info, unsigned long arg)
{
	struct nvc_param params;
	...
	case NVC_PARAM_FOCAL_LEN:
		info->nvc.focal_length = dw9716_FOCAL_LENGTH;
	...
	case NVC_PARAM_CAPS:
		...
		dw9716_get_focuser_capabilities(info);
...
static int dw9716_param_wr_s(struct dw9716_info *info,
		struct nvc_param *params, s32 s32val)
{
	...
	case NVC_PARAM_LOCUS:
		dev_dbg(&info->i2c_client->dev, "%s LOCUS: %d\n",
			__func__, s32val);
		err = dw9716_position_wr(info, s32val);
		break;
	case NVC_PARAM_RESET:
		err = dw9716_reset(info, s32val);
...
static int dw9716_param_wr(struct dw9716_info *info, unsigned long arg)
{
	struct nvc_param params;
	u8 u8val;
	s32 s32val;
	...
	case NVC_PARAM_STEREO:
		...
		case NVC_SYNC_SLAVE:
			if (info->s_info != NULL) {
				...
				err = dw9716_position_wr(info->s_info,
	...
	case NVC_PARAM_CAPS:
		if (dw9716_set_focuser_capabilities(info, &params)) {
			dev_err(&info->i2c_client->dev,
				"%s: Error: copy_from_user bytes %d\n",
	...
	default:
		...
		case NVC_SYNC_OFF:
		case NVC_SYNC_MASTER:
			return dw9716_param_wr_s(info, &params, s32val);
		case NVC_SYNC_SLAVE:
			return dw9716_param_wr_s(info->s_info, &params, s32val);
		case NVC_SYNC_STEREO:
			err = dw9716_param_wr_s(info, &params, s32val);
			if (!(info->pdata->cfg & NVC_CFG_SYNC_I2C_MUX))
				err |= dw9716_param_wr_s(info->s_info,
						&params, s32val);
			return err;
		default:
					...
					unsigned long arg)
{
	...
	case _IOC_NR(NVC_IOCTL_PARAM_WR):
		dw9716_pm_dev_wr(info, NVC_PWR_ON);
		err = dw9716_param_wr(info, arg);
		return err;
	case _IOC_NR(NVC_IOCTL_PARAM_RD):
		err = dw9716_param_rd(info, arg);
		return err;
...
static void dw9716_sdata_init(struct dw9716_info *info)
{
	/* set defaults */
	memcpy(&info->nvc, &dw9716_default_nvc, sizeof(info->nvc));
	memcpy(&info->cap, &dw9716_default_cap, sizeof(info->cap));
...
static int dw9716_sync_en(unsigned num, unsigned sync)
{
	struct dw9716_info *master = NULL;
	struct dw9716_info *slave = NULL;
	struct dw9716_info *pos = NULL;
	rcu_read_lock();
	list_for_each_entry_rcu(pos, &dw9716_info_list, list) {
...
static int dw9716_sync_dis(struct dw9716_info *info)
{
	if (info->s_info != NULL) {
...
static int dw9716_open(struct inode *inode, struct file *file)
{
	...
	if (!info)
		return -ENODEV;
	err = dw9716_sync_en(info->pdata->num, info->pdata->sync);
	if (err == -EINVAL)
		dev_err(&info->i2c_client->dev,
	...
	dw9716_pm_dev_wr(info, NVC_PWR_ON);
	dw9716_position_wr(info, info->cap.focus_infinity);
	dev_dbg(&info->i2c_client->dev, "%s\n", __func__);

	return 0;
}

static int dw9716_release(struct inode *inode, struct file *file)
{
	struct dw9716_info *info = file->private_data;
	dev_dbg(&info->i2c_client->dev, "%s\n", __func__);
	dw9716_pm_wr(info, NVC_PWR_OFF);
	file->private_data = NULL;
	WARN_ON(!atomic_xchg(&info->in_use, 0));
	if (info->s_info != NULL)
		WARN_ON(!atomic_xchg(&info->s_info->in_use, 0));
	dw9716_sync_dis(info);
	return 0;
}
...
#ifdef CONFIG_COMPAT
	.compat_ioctl = dw9716_ioctl,
#endif
	.release = dw9716_release,
};

static void dw9716_del(struct dw9716_info *info)
{
	dw9716_pm_exit(info);
	if ((info->s_mode == NVC_SYNC_SLAVE) ||
		(info->s_mode == NVC_SYNC_STEREO))
		dw9716_pm_exit(info->s_info);

	dw9716_sync_dis(info);
	spin_lock(&dw9716_spinlock);
	list_del_rcu(&info->list);
	spin_unlock(&dw9716_spinlock);
	synchronize_rcu();
}

static int dw9716_remove(struct i2c_client *client)
{
	struct dw9716_info *info = i2c_get_clientdata(client);
	dev_dbg(&info->i2c_client->dev, "%s\n", __func__);
...
static int dw9716_probe(
		struct i2c_client *client,
		const struct i2c_device_id *id)
{
	struct dw9716_info *info;
	int err;
	static struct regmap_config dw9716_regmap_config = {
		.reg_bits = 8,
		.val_bits = 16,
	};

	dev_dbg(&client->dev, "%s\n", __func__);
	pr_info("dw9716: probing focuser.\n");
	...
	if (client->dev.platform_data) {
		...
		info->pdata = &dw9716_default_pdata;
		dev_dbg(&client->dev, "%s No platform data.  Using defaults.\n",
			__func__);
	}

	info->regmap = devm_regmap_init_i2c(client, &dw9716_regmap_config);
	...
	spin_lock(&dw9716_spinlock);
	list_add_rcu(&info->list, &dw9716_info_list);
	spin_unlock(&dw9716_spinlock);
	dw9716_power_get(info);
	dw9716_sdata_init(info);
	if (info->pdata->cfg & (NVC_CFG_NODEV | NVC_CFG_BOOT_INIT)) {
		err = dw9716_detect(info);
		if (err < 0) {
	...
	if (info->pdata->dev_name != 0)
		...
		strncpy(info->devname, "dw9716", sizeof(info->devname) - 1);
	...
	info->miscdev.fops = &dw9716_fileops;
	...
	err = camera_dev_add_regmap(&info->csync_dev, "dw9716", info->regmap);
	if (err < 0) {
...
{
	struct dw9716_info *k_info = s->private;
...
set_attr:
	...
	case 'p':
		pr_info("new pos = %d\n", val);
		err = dw9716_position_wr(k_info, val);
		if (err)
...
{
	return i2c_add_driver(&dw9716_i2c_driver);
}
