...
struct palmas_ldousb_in_info {
	...
	u32 ldousb_in_vbus_threshold_voltage;
	struct mutex	mutex;
};

static struct palmas_ldousb_in_info *palmas_ldousb_in_dev;
...
reschedule:
	...
	mutex_unlock(&ldousb_info->mutex);
}

void palmas_ldousb_in_ldousb_voltage(int ldousb_voltage)
{
	struct palmas_ldousb_in_info *ldousb_info;
	int ret;
	int sel, input_next;

	if (!palmas_ldousb_in_dev)
		return;
	ldousb_info = palmas_ldousb_in_dev;

	if (ldousb_voltage < 0) {
		dev_err(ldousb_info->dev, "ldousb voltage is not proper\n");
		return;
	}

	mutex_lock(&ldousb_info->mutex);

	if (ldousb_voltage > ldousb_info->ldousb_in_vbus_threshold_voltage) {
		cancel_delayed_work(&ldousb_info->work);
		ldousb_info->work_disabled = true;

		if (ldousb_info->enable_in1_above_threshold)
			input_next = LDOUSB_INPUT1;
		else
			input_next = LDOUSB_INPUT2;
		...
		ret = regulator_enable(ldousb_info->input_reg[input_next]);
		if (ret < 0) {
			dev_err(ldousb_info->dev, "regulator enable for input 0 failed: %d\n",
				ret);
			goto reschedule;
		}
...
reschedule:
	ldousb_info->work_disabled = false;
	schedule_delayed_work(&ldousb_info->work,
		PALMAS_LDOSUB_IN_SEL_CHECK_DELAY);
	mutex_unlock(&ldousb_info->mutex);
}
EXPORT_SYMBOL_GPL(palmas_ldousb_in_ldousb_voltage);
...
static int palmas_ldousb_in_probe(struct platform_device *pdev)
{
	...
	if (palmas_pdata && palmas_pdata->ldousb_in_pdata) {
		...
		ldousb_info->ldousb_in_vbus_threshold_voltage =
				ldousb_pdata->ldousb_in_vbus_threshold_voltage;
	}
	...
	if (!ldousb_pdata && pdev->dev.of_node) {
		...
		ret = of_property_read_u32(node,
				"ti,ldousb-in-vbus-threshold-voltage", &pval);
		if (!ret)
			ldousb_info->ldousb_in_vbus_threshold_voltage = pval;
	...
	if (!ldousb_info->ldousb_in_vbus_threshold_voltage) {
		dev_err(&pdev->dev,
			"Invalid vbus threshold voltage, can not be 0\n");
		return -EINVAL;
	}
	...
	ldousb_info->current_switching_voltage =
				ldousb_info->ldousb_in_threshold_voltage;
	palmas_ldousb_in_dev = ldousb_info;
...
static int __init palmas_ldousb_in_init(void)
{
	return platform_driver_register(&palmas_ldousb_in_driver);
}

static void __exit palmas_ldousb_in_exit(void)
{
	platform_driver_unregister(&palmas_ldousb_in_driver);
}

late_initcall(palmas_ldousb_in_init);
module_exit(palmas_ldousb_in_exit);
